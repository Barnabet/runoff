...
describe("sectionUserPrompt — data block + rules block", () => {
  it("lands the section data block under the sources heading", () => {
    const prompt = sectionUserPrompt({ ...base, section: section([]) });
    expect(prompt).toContain("Sources bound to this section:\n### AR transactions (famA)\namount 12,340");
  });
  ...
  it("lists each rule's kind and text so the rules reach the model", () => {
    const prompt = sectionUserPrompt({
      ...
      section: section([
        ...
        { kind: "assert", text: "Spend must be positive.", sql: "SELECT SUM(amount) FROM fam_ar_transactions", op: ">", value: 0 },
      ]),
    });
    ...
    expect(prompt).toContain("- [assert] Spend must be positive. (sql: SELECT SUM(amount) FROM fam_ar_transactions)");
  ...
  it("collapses newlines in a multi-line assert SQL onto one line", () => {
    const prompt = sectionUserPrompt({
      ...base,
      section: section([{ kind: "assert", text: "", sql: "SELECT SUM(amount)\n  FROM fam_ar_transactions", op: ">", value: 0 }]),
    });
    expect(prompt).toContain("(sql: SELECT SUM(amount) FROM fam_ar_transactions)");
  });

  it("omits the sql suffix for an assert rule that has none", () => {
    const prompt = sectionUserPrompt({
      ...base,
...
describe("citation-marker wording", () => {
  ...
  it("the dialect contract shows a numeral placeholder and a concrete warehouse-table example", () => {
    const prompt = systemPrompt(content);
    expect(prompt).toContain("[[numeral|familyId|locator]]");
    expect(prompt).toContain("[[220,500|fam_ab12|sum(fam_ar_transactions.amount)]]");
    expect(prompt).not.toContain("[[figure|");
  });

  it("describes the agg(tableName.column) locator grammar against warehouse tables", () => {
    const prompt = systemPrompt(content);
    expect(prompt).toContain("locator is agg(tableName.column)");
    expect(prompt).toContain("fam_ar_transactions");
  });
  ...
  it("advertises row-filtered locators and bans internal-mechanics questions", () => {
    ...
    const prompt = systemPrompt(content);
    expect(prompt).toContain("sum(fam_ar_transactions.amount where channel=search)");
